...
#define FIRMWARE_VERSION "3.16.0"
...
uint8_t __attribute__((section(".ospi_data"))) sl_wfx_firmware[310352];

scan_result_list_t __attribute__((section(".ospi_data"))) scan_list[SL_WFX_MAX_SCAN_RESULTS];
	...
	void** event_payload_out)
{
	...
	do {
		...
		if ((wf200_interrupt_event == 1)
		    || ((control_register & SL_WFX_CONT_NEXT_LEN_MASK) != 0) || 1) {
...
#ifdef SL_WFX_DEBUG
  uPrintf("TX> ");
  uPrintf("%02X%02X %02X%02X ", ((char*)frame)[0], ((char*)frame)[1], ((char*)frame)[2], ((char*)frame)[3]);
  for(uint32_t i = 4; i < frame_len; i++)
  {
    uPrintf("%02X ", ((char*)frame)[i]);
  }
  uPrintf("\r\n");
#endif
...
void sl_wfx_process(void)
{
  ...
  {
    if(scan_ongoing == 0){
      uPrintf("!  # Ch RSSI MAC (BSSID)        Network (SSID)\r\n");
...
void sl_wfx_start_ap_callback(uint32_t status)
{
  if (status == 0) {
	  uPrintf("AP started\r\n");
	  uPrintf("Join the AP with SSID: %s\r\n", softap_ssid);
...
void sl_wfx_client_connected_callback(uint8_t* mac)
{
  ...
  uPrintf("Open a web browser and go to http://%d.%d.%d.%d\r\n",
...
void sl_wfx_ap_client_rejected_callback(uint32_t status, uint8_t* mac)
{
  ...
  uPrintf("Client rejected, reason: %d, MAC: %02X:%02X:%02X:%02X:%02X:%02X\r\n",
...
void sl_wfx_ap_client_disconnected_callback(uint32_t status, uint8_t* mac)
{
  ...
  memcpy(&mac_addr, mac, SL_WFX_BSSID_SIZE);
  dhcpserver_remove_mac(&mac_addr);
  uPrintf("Client disconnected, reason: %d, MAC: %02X:%02X:%02X:%02X:%02X:%02X\r\n",
         (int)status, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
