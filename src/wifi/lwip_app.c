...
char __attribute__((section(".ospi_data"))) string_list[4096];
char __attribute__((section(".ospi_data"))) event_log[50];
uint8_t ap_channel;
...
char __attribute__((section(".ospi_data"))) wlan_ssid[32];//                        = WLAN_SSID_DEFAULT;
char __attribute__((section(".ospi_data"))) wlan_passkey[64];//                     = WLAN_PASSKEY_DEFAULT;
sl_wfx_security_mode_t wlan_security      = WLAN_SECURITY_DEFAULT;
char __attribute__((section(".ospi_data"))) softap_ssid[32];//                      = SOFTAP_SSID_DEFAULT;
char __attribute__((section(".ospi_data"))) softap_passkey[64];//                   = SOFTAP_PASSKEY_DEFAULT;
...
extern uint16_t WifiReceived;
extern bool WifiConnected;
extern bool HostConnecting;
extern char WifiPayload[];
                                             ...
                                             char *pc_param[], char *pc_value[])
{
  ...
  if (num_params == 3) {
    ...
    if (!(wifi.state & SL_WFX_STA_INTERFACE_CONNECTED))
    {
      ...
      {
        uPrintf("Connection command error\r\n");
    ...
    uPrintf("Invalid Connection Request\r\n");
  ...
  case 2: /* <!--#scan_list--> */
    for(int i = 0; i < scan_count_web; i++)
    {
      snprintf(string_field, 100, "{\"ssid\":\"%s\", \"rssi\":\"%d\", \"secu\":\"%s\"}",
		    (strlen((char *) scan_list[i].ssid_def.ssid) != 0) ? (char *) scan_list[i].ssid_def.ssid : "Hidden",
...
void sl_lwip_init(void)
{
  ...
  uPrintf("FMAC Driver version    %s\r\n", FMAC_DRIVER_VERSION_STRING);
  switch(status) {
  case SL_STATUS_OK:
    wifi.state = SL_WFX_STARTED;
    uPrintf("WF200 Firmware version %d.%d.%d\r\n", 
           wifi.firmware_major,
           wifi.firmware_minor,
           wifi.firmware_build);
    uPrintf("WF200 initialization successful\r\n");
    break;
  case SL_STATUS_WIFI_INVALID_KEY:
    uPrintf("Failed to init WF200: Firmware keyset invalid\r\n");
    break;
  case SL_STATUS_WIFI_FIRMWARE_DOWNLOAD_TIMEOUT:
    uPrintf("Failed to init WF200: Firmware download timeout\r\n");
    break;
  case SL_STATUS_TIMEOUT:
    uPrintf("Failed to init WF200: Poll for value timeout\r\n");
    break;
  case SL_STATUS_FAIL:
    uPrintf("Failed to init WF200: Error\r\n");
    break;
  default :
    uPrintf("Failed to init WF200: Unknown error\r\n");
  }
...
err_t RecvHttpHeaderCallback(httpc_state_t *connection,
	void *arg,
	struct pbuf *hdr,
	u16_t hdr_len,
	u32_t content_len) 
{
	...
	uPrintf("RecvHttpHeaderCallback:\r\nLen: %u\r\n", hdr_len);
	uPrintf("%s\r\n", hdr->payload);
...
void HttpClientResultCallback(void *arg,
	httpc_result_t httpc_result,
	u32_t rx_content_len,
	u32_t srv_res,
	err_t err) 
{
//	uPrintf("HttpClientResultCallback has been called\r\n");
//	uPrintf("httpc_result: %u\r\n", httpc_result);
//	uPrintf("received number of bytes: %lu\r\n", rx_content_len);
}
...
err_t my_http_cb(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
	...
	memcpy(WifiPayload, p->payload, p->len);
	WifiReceived = p->len;
	//uPrintf(WifiPayload);
	//uPrintf("%u\r\n", p->len);
	pbuf_free(p);
...
void sl_lwip_process(void)
{
	...
	if (HAL_GetTick() - (Times + 100))
	{
		...
		if ((wifi.state & SL_WFX_STA_INTERFACE_CONNECTED) != SL_WFX_STA_INTERFACE_CONNECTED)
		{
			...
			sl_wfx_send_join_command((uint8_t*) wlan_ssid, strlen(wlan_ssid), NULL,
				0, wlan_security, 1, 0, (uint8_t*) wlan_passkey, strlen(wlan_passkey),
				NULL, 0);
		}
		else 
		{
	...
	if (WifiConnected && HostConnecting && !FileReq)
	{
		//FileReq = true;
		conn_settings.use_proxy = 0; 
		//conn_settings.headers_done_fn = RecvHttpHeaderCallback; 
		conn_settings.headers_done_fn = NULL;
		//conn_settings.result_fn = result_fn;
		conn_settings.result_fn = NULL;
	
		
		error = httpc_get_file_dns("do.pripyachka.com", 80, TxMsg, &conn_settings, my_http_cb, 0, &connection);
		
		if (error == ERR_OK)
		{
			FileReq = true;
		}
		else uPrintf("State: %d\r\n", error);
