                                                  ...
                                                  uint16_t buffer_length)
{
  ...
  HAL_SPI_Transmit(&hspi3, header, header_length, 1000);
  if(is_read)
  {
    if(HAL_SPI_Receive(&hspi3, buffer, buffer_length, 50) == HAL_OK) result = SL_STATUS_OK;
  }else{
    if(HAL_SPI_Transmit(&hspi3, buffer, buffer_length, 50) == HAL_OK) result = SL_STATUS_OK;
  }
